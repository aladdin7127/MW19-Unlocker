...
async def mines(ctx, round_id):
    ...
    if round_length < 36:
        ...
        em = discord.Embed(color=0x1A1367)
        em.set_footer(text="BF Predictor")
        em.add_field(name="Prediciton:\n",value=row1 + "\n" + row2 + "\n" + row3 + "\n" + row4 +"\n" + row5 + "\n" + "**Accuracy**" + "\n" + info +"%")
...
async def crash(ctx):
    ...
    em = discord.Embed(color=0x1A1367)
    em.set_footer(text="BF Predictor")
    em.add_field(name=f"**Prediction: {prediction}**", value=f"Average: {average}\nSafe Bet: {safe}")
    await ctx.reply(embed=em)

@bot.command()
async def commands(ctx):
    await ctx.send("```/mines [RoundID] - Predicts mine locations\n/crash - Predicts crash```")

bot.run("MTAzNDEwNDM0ODA0MjYxMjc3Ng.GqFPsI.5OnJd8z-bq5LYf0Nhfyftl2wlgnytWvtrdaciE")